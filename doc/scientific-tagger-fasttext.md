...
We propose a mixed approach, leveraging on embeddings technique in NLP to train classifiers with article metadata (title, abstract, keywords in particular) labeled with the journal-level classification FoR (Fields of Research) and then apply these classifiers at the article level. <br>
...
Several data providers already provide classifications, but one of the rules we have set for ourselves in this work [@coso_feedback_2018] is to apply open science principles to monitor open science. Accordingly, we want to use and re-release open data, and use and redistribute open code. In this sense, using classifications from non-open sources was not an option.
...
Going to a publication-level classification brings several advantages: first, it should allow classifying every type of document, published in a well-established journal or as a preprint. Also, it should handle better articles published in multidisciplinary journals like Science or Nature (even though a single article can by itself be classified as multidisciplinary). <br>
...
This source presents the merits of being quite recent, with a hierarchical structure, and produced by a public organization.
...
Getting labeled data at the article level is very costly (high level of domain-specific expertise for a huge amount of publications). At the same time, label data does exist at the journal level. We try here to leverage that data to produce an article-level classifier.

The idea is to extract relevant pieces of information from available metadata (title, abstract, keyword, MeSH) to classify a publication. We assume that the publications of journals tagged with a given Field of Research, "Chemical Sciences" for example, will contain field-specific words and n-gram in their metadata that will be caught by a machine learning classifier. As a consequence, if an article from, say, a "Multidisciplinary" journal contains enough words specific to "Chemical Sciences", then we guess that a machine learning approach will be able to classify it as "Chemical Sciences" rather than "Multidisciplinary". In that case, the classification at the article level would be different from the one at the journal level.
...
First, it appears that 18% of the publications in PubMed cannot be attached directly to an FoR code as their ISSN is not ERA data giving a correspondence between ISSN and FoR codes.
